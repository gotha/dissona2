...
async def handle_job(msg, processor):
    """Handle a single NATS job message."""
    job = None
    try:
        ...
        job_id = job.get("job_id") if job else "unknown"
        logger.error("Job failed", error=str(e), job_id=job_id)
        await msg.nak()
...
async def main():
    ...
    try:
        ...
        logger.warning("Metrics port already in use, skipping", port=settings.metrics_port)
